...
NSString * const SINetworkAccessibityChangedNotification = @"SINetworkAccessibityChangedNotification";

typedef NS_ENUM(NSInteger, SINetworkType) {
    SINetworkTypeUnknown ,
    SINetworkTypeOffline ,
    SINetworkTypeWiFi    ,
    SINetworkTypeCellularData ,
};

@interface SINetworkAccessibity(){
    SCNetworkReachabilityRef _reachabilityRef;
    CTCellularData *_cellularData;
    NSMutableArray *_becomeActiveCallbacks;
    SINetworkAccessibleState _previousState;
...
@implementation SINetworkAccessibity
#pragma mark - Public
...
+ (SINetworkAccessibity *)sharedInstance {
    static SINetworkAccessibity * instance = nil;
...
- (void)monitorNetworkAccessibleStateWithCompletionBlock:(NetworkAccessibleStateNotifier)block {
...
- (void)setupNetworkAccessibity {
    ...
    BOOL firstRun = ({
        static NSString * RUN_FLAG = @"SINetworkAccessibityRunFlag";
...
- (void)cleanNetworkAccessibity {
    ...
    _previousState = SINetworkChecking;
...
static void ReachabilityCallback(SCNetworkReachabilityRef target, SCNetworkReachabilityFlags flags, void* info) {
    SINetworkAccessibity *networkAccessibity = (__bridge SINetworkAccessibity *)info;
    if (![networkAccessibity isKindOfClass: [SINetworkAccessibity class]]) {
        return;
    }
...
- (void)startCheck {
    if ([UIDevice currentDevice].systemVersion.floatValue < 10.0 || [self currentReachable]) {
        ...
        /* 先用 currentReachable 判断，若返回的为 YES 则说明：
         ...
         此时是有网络访问权限的，直接返回 SINetworkAccessible
         **/
        
        [self notiWithAccessibleState:SINetworkAccessible];
        return;
    }
    ...
    switch (state) {
        case kCTCellularDataRestricted: {// 系统 API 返回 无蜂窝数据访问权限
            ...
            [self getCurrentNetworkType:^(SINetworkType type) {
                /*  若用户是通过蜂窝数据 或 WLAN 上网，走到这里来 说明权限被关闭**/
                
                if (type == SINetworkTypeCellularData || type == SINetworkTypeWiFi) {
                    [self notiWithAccessibleState:SINetworkRestricted];
                } else {  // 可能开了飞行模式，无法判断
                    [self notiWithAccessibleState:SINetworkUnknown];
                }
            }];
...
- (void)getCurrentNetworkType:(void(^)(SINetworkType))block {
    if ([self isWiFiEnable]) {
        return block(SINetworkTypeWiFi);
    }
    SINetworkType type = [self getNetworkTypeFromStatusBar];
    if (type == SINetworkTypeWiFi) { // 这时候从状态栏拿到的是 Wi-Fi 说明状态栏没有刷新，延迟一会再获取
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
...
- (SINetworkType)getNetworkTypeFromStatusBar {
    NSInteger type = 0;
    @try {
        ...
        if (isModernStatusBar) { // 在 iPhone X 上 statusBar 属于 UIStatusBar_Modern ，需要特殊处理
            ...
            return  wifiEnable     ? SINetworkTypeWiFi :
            cellularEnable ? SINetworkTypeCellularData : SINetworkTypeOffline;
        } else { // 传统的 statusBar
            ...
            return type == 0 ? SINetworkTypeOffline :
            type == 5 ? SINetworkTypeWiFi    : SINetworkTypeCellularData;
        }
...
- (void)notiWithAccessibleState:(SINetworkAccessibleState)state {
    if (_automaticallyAlert) {
        if (state == SINetworkRestricted) {
    ...
    if (state != _previousState) {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:SINetworkAccessibityChangedNotification object:nil];

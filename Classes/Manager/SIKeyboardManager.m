...
static int _SIKeyboardViewFrameObserverKey;
...
@implementation _SIKeyboardViewFrameObserver {
...
- (void)addToKeyboardView:(UIView *)keyboardView {
    ...
    if (_keyboardView) {
        ...
        objc_setAssociatedObject(_keyboardView, &_SIKeyboardViewFrameObserverKey, nil, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    }
    _keyboardView = keyboardView;
    if (keyboardView) {
        [self addFrameObserver];
    }
    objc_setAssociatedObject(keyboardView, &_SIKeyboardViewFrameObserverKey, self, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
...
+ (instancetype)observerForView:(UIView *)keyboardView {
    ...
    return objc_getAssociatedObject(keyboardView, &_SIKeyboardViewFrameObserverKey);
}
...
@implementation SIKeyboardManager {
...
- (instancetype)init {
    @throw [NSException exceptionWithName:@"SIKeyboardManager init error" reason:@"Use 'defaultManager' to get instance." userInfo:nil];
    return [super init];
}
...
- (void)_initFrameObserver {
    ...
    _SIKeyboardViewFrameObserver *observer = [_SIKeyboardViewFrameObserver observerForView:keyboardView];
    if (!observer) {
        observer = [_SIKeyboardViewFrameObserver new];
...
- (void)_notifyAllObservers {
    ...
    if (!CGRectEqualToRect(trans.toFrame, _fromFrame)) {
        for (id<SIKeyboardObserver> observer in _observers.copy) {

...
NSString * const SIDownloadProgressDidChangeNotification = @"SIDownloadProgressDidChangeNotification";
NSString * const SIDownloadStateDidChangeNotification = @"SIDownloadStateDidChangeNotification";
NSString * const SIDownloadInfoKey = @"SIDownloadInfoKey";
...
/** 根文件夹 */
static NSString * const SIDownloadRootDir = @"si-downloadRoot";
/** 默认manager的标识 */
static NSString * const SIDowndloadManagerDefaultIdentifier = @"si-downloadmanager";
...
@interface SIDownloadInfo(){
    SIDownloadState _state;
    NSInteger _totalBytesWritten;
}

/** 下载状态 */
@property (assign, nonatomic) SIDownloadState state;
/** 这次写入的数量 */
...
/** 存放所有的进度回调 */
@property (copy, nonatomic) SIDownloadProgressChangeBlock progressChangeBlock;
/** 存放所有的完毕回调 */
@property (copy, nonatomic) SIDownloadStateChangeBlock stateChangeBlock;
/** 任务 */
...
@implementation SIDownloadInfo
- (NSString *)file{
    if (_file == nil) {
        _file = [[NSString stringWithFormat:@"%@/%@", SIDownloadRootDir, self.filename] prependCaches];
    }
...
- (SIDownloadState)state
{
    // 如果是下载完毕
    if (self.totalBytesExpectedToWrite && self.totalBytesWritten == self.totalBytesExpectedToWrite) {
        return SIDownloadStateCompleted;
    }
    
    // 如果下载失败
    if (self.task.error) return SIDownloadStateNone;
...
- (void)notifyProgressChange
{
    ...
    [DownloadNoteCenter postNotificationName:SIDownloadProgressDidChangeNotification
                                        object:self
                                      userInfo:@{SIDownloadInfoKey : self}];
}
...
- (void)notifyStateChange
{
    ...
    [DownloadNoteCenter postNotificationName:SIDownloadStateDidChangeNotification
                                        object:self
                                      userInfo:@{SIDownloadInfoKey : self}];
}

#pragma mark - 状态控制
- (void)setState:(SIDownloadState)state
{
    SIDownloadState oldState = self.state;
...
- (void)cancel
{
    if (self.state == SIDownloadStateCompleted || self.state == SIDownloadStateNone) return;
    
    [self.task cancel];
    self.state = SIDownloadStateNone;
}
...
- (void)resume
{
    if (self.state == SIDownloadStateCompleted || self.state == SIDownloadStateResumed) return;
    
    [self.task resume];
    self.state = SIDownloadStateResumed;
}
...
- (void)willResume
{
    if (self.state == SIDownloadStateCompleted || self.state == SIDownloadStateWillResume) return;
    
    self.state = SIDownloadStateWillResume;
}
...
- (void)suspend
{
    if (self.state == SIDownloadStateCompleted || self.state == SIDownloadStateSuspened) return;
    
    if (self.state == SIDownloadStateResumed) { // 如果是正在下载
        [self.task suspend];
        self.state = SIDownloadStateSuspened;
    } else { // 如果是等待下载
        self.state = SIDownloadStateNone;
    }
}
...
- (void)didCompleteWithError:(NSError *)error
{
    ...
    // 通知(如果下载完毕 或者 下载出错了)
    if (self.state == SIDownloadStateCompleted || error) {
        // 设置状态
        self.state = error ? SIDownloadStateNone : SIDownloadStateCompleted;
    }
}
@end

@interface SIDownloadManager() <NSURLSessionDataDelegate>
/** session */
...
{
    return [self managerWithIdentifier:SIDowndloadManagerDefaultIdentifier];
}
...
+ (instancetype)managerWithIdentifier:(NSString *)identifier
{
    ...
    SIDownloadManager *mgr = _managers[identifier];
    if (!mgr) {
        mgr = [self manager];
...
- (SIDownloadInfo *)download:(NSString *)url toDestinationPath:(NSString *)destinationPath progress:(SIDownloadProgressChangeBlock)progress state:(SIDownloadStateChangeBlock)state
{
    ...
    // 下载信息
    SIDownloadInfo *info = [self downloadInfoForURL:url];
    ...
    // 如果已经下载完毕
    if (info.state == SIDownloadStateCompleted) {
        // 完毕
        [info notifyStateChange];
        return info;
    } else if (info.state == SIDownloadStateResumed) {
        return info;
    }
...
- (void)resumeFirstWillResume
{
    ...
    SIDownloadInfo *willInfo = [self.downloadInfoArray filteredArrayUsingPredicate:[NSPredicate predicateWithFormat:@"state==%d", SIDownloadStateWillResume]].firstObject;
    [self resume:willInfo.url];
}

- (void)cancelAll
{
    [self.downloadInfoArray enumerateObjectsUsingBlock:^(SIDownloadInfo *info, NSUInteger idx, BOOL *stop) {
...
- (void)resume:(NSString *)url
{
    ...
    // 正在下载的
    NSArray *downloadingDownloadInfoArray = [self.downloadInfoArray filteredArrayUsingPredicate:[NSPredicate predicateWithFormat:@"state==%d", SIDownloadStateResumed]];
...
- (SIDownloadInfo *)downloadInfoForURL:(NSString *)url
{
    ...
    SIDownloadInfo *info = [self.downloadInfoArray filteredArrayUsingPredicate:[NSPredicate predicateWithFormat:@"url==%@", url]].firstObject;
    if (info == nil) {
        info = [[SIDownloadInfo alloc] init];
        info.url = url; // 设置url
...
{
    // 获得下载信息
    SIDownloadInfo *info = [self downloadInfoForURL:dataTask.taskDescription];
...
{
    // 获得下载信息
    SIDownloadInfo *info = [self downloadInfoForURL:task.taskDescription];
